...
```bash
head -1 ~/.hermes/proposals/projects.csv
# Expected: id,name,proposal_count,git_repo,local_path,prj_url,description,last_update
...
ls -t ~/.hermes/proposals/backups/

# Check backup structure before restoring
head -1 ~/.hermes/proposals/backups/backup_YYYYMMDD_HHMMSS/projects.csv
...
mapping.csv:     project_id,project_name,project_git_repo,project_local_path,proposal_id,
                 proposal_name,proposal_status
```
