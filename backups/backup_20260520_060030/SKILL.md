...
4. 委托 dev agent（delegate_task，超时600s）
   - dev 成功：自动 commit + push
   - dev 超时：主 agent 接管验收 → 手动 commit + push
5. 构建验证（npx vite build，20-35s）
6. 更新 proposal-index.md（Stage=accepted, Acceptance=accepted, Notes含commit）
7. 立即展示下一轮方向选项（A/B/C/D/E）
8. 回到步骤2，循环
```
...
| `local_path` | See `references/local-path-population.md` — symlink strategy |
...
cd /home/hermes/.hermes/proposals && git checkout -- proposals.csv
...
| references/data-recovery.md | Data recovery procedures |
