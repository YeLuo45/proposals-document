...
- project_proposal_mapping.csv: project_id, project_name, project_git_repo, project_local_path,
                               proposal_id, proposal_name, proposal_status
"""
...
PROPOSALS_CSV_HEADERS = ['id', 'title', 'owner', 'status', 'project_id', 'project_name', 'stage',
                          ...
                          'deployment_branch', 'prd_confirmation', 'tech_expectations', 'acceptance',
                          'research_direction', 'last_update', 'engine', 'target', 'game_type', 'notes']
MAPPING_CSV_HEADERS = ['project_id', 'project_name', 'project_git_repo', 'project_local_path',
                        'proposal_id', 'proposal_name', 'proposal_status']
...
def cmd_delete_project(args):
    ...
    mappings = [m for m in mappings if m.get('project_id') != args.id]
    if mappings:
        write_csv(MAPPING_CSV, MAPPING_CSV_HEADERS, mappings)
...
def cmd_update_proposal(args):
    ...
    # Update mapping if project changed
    if args.project_id and args.project_id != old_project_id:
        _, mappings = load_mapping()
        for m in mappings:
            if m.get('proposal_id') == args.id:
                m['project_id'] = args.project_id
                proj = get_project_by_id(args.project_id, projects)
...
def cmd_delete_proposal(args):
    ...
    # Remove from mapping
    _, mappings = load_mapping()
    mappings = [m for m in mappings if m.get('proposal_id') != args.id]
    if mappings:
        write_csv(MAPPING_CSV, MAPPING_CSV_HEADERS, mappings)
